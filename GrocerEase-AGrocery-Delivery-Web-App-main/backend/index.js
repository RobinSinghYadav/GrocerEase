...
const storage = multer.diskStorage({
    ...
    filename: (req, file, cb) => {
        return cb(null, `${file.fieldname}_${Date.now()}${path.extname(file.originalname)}`); // returns new name for uploaded image
    }
});
...
const fetchUser = async (req, res, next) => {
    ...
    try {
        const data = jwt.verify(token, process.env.JWT_SECRET); // Use the secret from .env
        req.user = data.user;
        next();
    } catch (error) {
        return res.status(401).send({ errors: "Please Authenticate Using a Valid Token" });
...
// Creating API for adding products in cart data in db
...
app.post('/removefromcart', fetchUser, async (req, res) => {
    ...
    let userData = await Users.findOne({ _id: req.user.id }); // finding the user id generated by the MongoDB
    if (userData.cartData[req.body.itemId] > 0)
        userData.cartData[req.body.itemId] -= 1;
    await Users.findOneAndUpdate({ _id: req.user.id }, { cartData: userData.cartData }); // updating the user's cart item in db
    res.send("Removed");
});
